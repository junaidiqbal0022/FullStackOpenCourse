...
var Fn = '/api/persons',
  ...
  Rn = async (e) => (await P.post(`${Fn}/add`, e)).data,
  zn = (e, t) => P.put(`${Fn}/update/${e}`, t).then((e) => e.data),
  Bn = (e) => P.delete(`${Fn}/delete/${e}`).then((e) => e.data),
  Vn = ({ errorMessage: e }) =>
    e
    ...
    children: (0, m.jsx)(() => {
      ...
      (0, d.useEffect)(() => {
        ...
        In()
          ...
          .catch((e) => {
            h('failed to fetch data from server ', e)
          }))
      }, [])
      let y = (n) => {
            ...
            zn(r.id, n)
              ...
              .catch((e) =>
                e.response?.status === 404
                  ? (h(`Person ${n.name} was not found on server side.`), !0)
                  : (h(`failed to update ${n.name} in server`), !0),
              )
            return
          ...
          Rn(n)
            ...
            .catch((e) => {
              if (
                ...
                h(`Person with name ${n.name} already exists on server.`),
                Ln(n.name)
                  .then(async (e) => {
                    if (!e) {
                      h(
                        `Person with name ${n.name} not found on server after duplicate entry error.`,
                      )
                      return
                    }
                    (t((t) => [...t.filter((t) => t.name !== e.name), e]),
                    h(
                      `Person with name ${n.name} already exists on server. Updated local data with server data.`,
                    ))
                  })
                  .catch((e) => {
                    (h(
                      `Failed to fetch existing person data for ${n.name} from server.`,
                    ),
                    h(`failed to update local data for ${n.name} `, e))
                  }))
                return
              } else {
                h('failed to update local data ', e)
                return
              }
            })
        },
          ...
          Bn(r)
            ...
            .catch((e) => {
              if (e.response?.status === 404) {
                h(`Person ${n} was not found on server side.`)
                return
              }
              h(`failed to delete ${i} from server`)
            }))
        }
      return (0, m.jsxs)('div', {
        children: [
